...
def generate_launch_description():
    ...
    payload_mass_arg = DeclareLaunchArgument(
        "payload_mass",
        default_value="5.0",
        description="Payload mass in kg. (In PPO mode can be overridden by /payload_mass topic.)",
    )
    ...
    run_id_arg = DeclareLaunchArgument(
        "run_id",
        default_value="real_test",
        description="Identifier string stored in the CSV log."
    )

    # 1) Speed supervisor node (non-PPO)
    ...
    # 2) Cycle signals node (REAL: scaled controller status topic)
    cycle_signals = Node(
        package="ur5e_cell_bringup",
    ...
    cycle_logger = Node(
        ...
        parameters=[
                ...
                {"run_id": run_id},
                {"payload_mass": payload_mass},
                {"real": True},                 # <<< this is the key
                {"use_estimated_effort": True}, # estimator only used if ALL real efforts are ~zero (because real=True)
                {"use_sim_time": False},
            ],
        condition=UnlessCondition(is_ppo),
    )
    ...
    cycle_logger_ppo = Node(
        ...
        parameters=[
            ...
            {"run_id": run_id},
            {"payload_mass": 0.0},  # overridden by /payload_mass from PPO if you publish it
            {"use_sim_time": False},
        ],
        condition=IfCondition(is_ppo),
    )

    # 3) MoveIt speed executor (delayed start) — REAL: scaled controller action
